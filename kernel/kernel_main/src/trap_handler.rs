...
use kernel_cpu::{
    csr::{XCAUSE_DESCRIPTION},
    read_scause, read_sip, read_sscratch, read_sstatus, read_stval, write_sie, write_sip,
};
use kernel_paging::{EntryBits, Paging, Sv39};
use kernel_process::Process;
use kernel_trap_frame::TrapFrame;

use crate::{
    loop_forever_black_box, phys_to_virt, syscall::handle_syscall, virt_to_phys,
    HartLocals, GAP,
};
...
pub fn handle_interrupt(mut process: Option<&mut Process>, cause: usize) {
    ...
    match cause {
        ...
        SUPERVISOR_EXTERNAL => {
            ...
            if let Some(e) = HartLocals::current()
                .interrupt_notifiers
                .borrow_mut()
                .remove(&(id as usize))
            {
...
pub unsafe fn trap_handler() {
    ...
    trap_frame.sie = 0;
    let mut switch_to_trap_frame =
        ((*read_sscratch()).restore_context as *const TrapFrame as *mut TrapFrame).as_mut();
    if let Some(_e) = &mut switch_to_trap_frame {
        if is_interrupt {
            //e.sie = 0;
        }
    }
    ...
    if is_interrupt {
        ...
        let permissions = if !is_supervisor {
            EntryBits::USER as u8
        } else {
            0
        };
        if tval != 0 {
            ...
            println!(
                "{:#x} -> {:?}",
                tval,
                sv39.query_permissions(tval, permissions)
            );
        } else {
            println!("TVAL = 0");
        }
